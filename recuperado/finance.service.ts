...
export class FinanceService {
  ...
  payInvoice(detId: number, clientName: string): Observable<any> {
    return this.http.post<any>(`${this.env.rootUrl}api/facturation/invoices/${detId}/pay`,
      JSON.stringify({ client_name: clientName }), { headers: this.getHeaders() });
  }

  abonarInvoice(detId: number, amount: number, clientName: string): Observable<any> {
    return this.http.post<any>(`${this.env.rootUrl}api/facturation/invoices/${detId}/abonar`,
      JSON.stringify({ amount, client_name: clientName }), { headers: this.getHeaders() });
  }
  ...
  createEgresoV2(data: { concept: string; category: string; value: number }): Observable<any> {
